...
class VideoReferMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # ========== NEW: Add mask decoder ==========
            from .mask_decoder import MaskDecoder
            self.mask_decoder = MaskDecoder(
                transformer_dim=getattr(config, 'mm_hidden_size', 256),
                transformer=None,  # Will be set in initialize_vision_modules
                decoding_type='query',
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # ========== NEW: Initialize mask decoder ==========
        from .mask_decoder import MaskDecoder
        from .two_way_transformer import TwoWayTransformer
        
        if getattr(self, 'mask_decoder', None) is None:
            self.mask_decoder = MaskDecoder(
                transformer_dim=getattr(self.config, 'mm_hidden_size', 256),
                transformer=None,
                decoding_type='query',
                use_temp_objectness=True,
            )
        else:
            # In case it is frozen by LoRA/checkpoint
            for p in self.mask_decoder.parameters():
                p.requires_grad = True
        
        # Attach TwoWayTransformer (critical!)
        if getattr(self.mask_decoder, 'transformer', None) is None:
            self.mask_decoder.transformer = TwoWayTransformer(
                depth=2,
                embedding_dim=self.config.mm_hidden_size,
                num_heads=8,
                mlp_dim=2048,
            )
        # ==================================================
...
class VideoReferMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, masks, frame, ann_indices, frame_nums
    ):
