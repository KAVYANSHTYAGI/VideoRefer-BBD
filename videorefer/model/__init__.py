...
def build_mask_decoder(config, delay_load=True):
    """
    Build a MaskDecoder instance for GROVE-style grounding.
    
    If delay_load=True, the transformer argument will be None 
    and should be assigned later (after constructing TwoWayTransformer).
    
    Config parameters (with defaults):
        - mask_decoder_transformer_dim: 256
        - mask_decoder_num_multimask_outputs: 3
        - mask_decoder_decoding_type: "query"
        - mask_decoder_use_temp_objectness: True
    """
    transformer_dim = getattr(
        config, "mask_decoder_transformer_dim", 
        getattr(config, "mm_hidden_size", 256)
    )
    num_multimask_outputs = getattr(config, "mask_decoder_num_multimask_outputs", 3)
    decoding_type = getattr(config, "mask_decoder_decoding_type", "query")
    use_temp_objectness = getattr(config, "mask_decoder_use_temp_objectness", True)

    # If delaying load of heavy transformer, pass None; user must set decoder.transformer later
    transformer = None if delay_load else getattr(config, "two_way_transformer", None)
